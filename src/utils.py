...
def load_json_by_mode(path: Union[str, Path], mode: Optional[str] = None):
    path = str(path)

    if mode == "dataframe":
        # Simple DataFrame load using pandas.read_json.
    ...
    # Normal JSON mode (raw Python objects)
    with open(path, "r", encoding="utf-8") as f:
        text = f.read()

    try:
        return json.loads(text)  # standard JSON
    except json.JSONDecodeError:
        # Fallback: JSON Lines -> list of JSON objects
