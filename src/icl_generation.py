...
def challenge_generator_func(verifier_system_prompt, generation_history, generation_command, words, domains, previous_gate_number=0, model = "o3-mini"):
    ...
    generator_output = {
        ...
        'generator response': generator_response  # keep for later use
    }
...
def generate_icl_examples(domains, words, minimum_problem_per_difficulty=10):
    ...
    # Progress bar: unknown total, so we track gates processed and show live counts.
    with tqdm(total=None, desc="Generating ICL examples", unit=" gate") as pbar:
        while (easy_problems < minimum_problem_per_difficulty or 
            medium_problems < minimum_problem_per_difficulty or 
            difficult_problems < minimum_problem_per_difficulty):

            random_elements = random.sample(words, 50)
            ...
            if (utils.solution_verification(prover_response_gpt4o, problem)):
                gpt4o_correct_responses.append(prover_response_gpt4o["response dict"])
                gpt4o_correct = True
            ...
            if (utils.solution_verification(prover_response_o3_mini, problem)):
                o3_mini_correct_responses.append(prover_response_o3_mini["response dict"])
            ...
            if (utils.solution_verification(prover_response_o3, problem)):
                o3_correct_responses.append(prover_response_o3["response dict"])
                o3_correct = True
            ...
            if gpt4o_correct and o3_mini_correct and o3_correct:
                ...
                if difficult_problems >= minimum_problem_per_difficulty:
                    current_verifier_command = prompts.generator_difficult_commands["Reduce"]                    

            prev_gate_number += 1

            # update the progress bar every gate
            pbar.update(1)
            pbar.set_postfix(easy=easy_problems, medium=medium_problems, hard=difficult_problems)

            if (prev_gate_number % 5 == 0):
                pbar.write("Some progress has been been made")
                
                with open('ICL-examples/low_reasoning.json', 'w')  as file:
                    json.dump(gpt4o_correct_responses, file)
                with open('ICL-examples/medium_reasoning.json', 'w')  as file:
                    json.dump(o3_mini_correct_responses, file)
                with open('ICL-examples/high_reasoning.json', 'w')  as file:
                    json.dump(o3_correct_responses, file)
                with open('ICL-examples/verifier_outputs.json', 'w')  as file:
                    json.dump(verifier_outputs, file)
                with open('ICL-examples/verifier_history.json', 'w')  as file:
                    json.dump(verifier_history, file)  
                with open('ICL-examples/prover_response_dict.json', 'w')  as file:
                    json.dump(prover_response_dict, file)
