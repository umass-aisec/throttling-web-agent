...
def construct_system_prompt(domains, example_directory="ICL-examples"):
    ...
    medium = utils.load_json_by_mode(example_directory + "/" + required_files[1])
    high = utils.load_json_by_mode(example_directory + "/" + required_files[2])

    simple_gates = list(set(easy) & set(medium) & set(high))
    medium_gates = list(set(medium) & set(high) - set(easy))
    difficult_gates = list(set(high) - set(easy) - set(medium))
